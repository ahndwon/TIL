...
```java
clas User constructor(_nickname: String) {
	val nickname: String
	init {
		nickname = _nickname
	}
...
- 복사본은 원본과 다른 생명주기를 가지며, 복사를 하면서 일부 프로퍼티 값을 바꾸거나 복사본을 제거해도 프로그램에서 원보을 참조하는 다른 부분에 전혀 영향을 끼치지 않는다.
```
...
- **Decorator Pattern** : 상속을 허용하지 않는 클래스(기존 클래스) 대신 사용할 수 있는 새로운 클래스(Decorator)를 통해 기존 클래스와 같은 인터페이스를 데코레이터가 재공하고, 기존 클래스를 데코레이터 내부에 필드로 유지하는 패턴.
...
### 4.4.4 객체 식: 무명 내부 클래스를 다른 방식을 ㅗ작성
```java
